...
- **Modal gallery card stretch:** `GalleryItem` in `ProjectModal.tsx` applies a cursor-parallax effect on non-phone cards — `mousemove` translates the inner image up to ±6px/±5px toward the cursor and scales it to `1.06×` (the card border frame stays fixed, `overflow-hidden` clips the edges cleanly). `mouseleave` springs back over 0.55s with expo-out easing. `willChange: 'transform'` on the image wrapper. Gated on `usePrefersReducedMotion`, skipped for phone-frame gallery items.
