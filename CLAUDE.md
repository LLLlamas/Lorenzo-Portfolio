...
| A project highlight | `highlights` on each `Project` is now typed `Highlight[]` (not `string[]`). Each entry is `{ text: string; metric?: string }`. When `metric` is provided, it renders as a large `font-display text-2xl` stat above `text` — ideal for shipping-time, rating, or count callouts. Leave `metric` absent when you only have descriptive text. |
...
- **Modal gallery** (`ProjectModal.tsx`): gallery images now render in a horizontal scroll-snap filmstrip (`-mx-5 md:-mx-8 flex snap-x snap-mandatory overflow-x-auto scrollbar-none`), not a grid. Phone shots get `w-[48vw] md:w-48` wrappers; browser shots get `w-[75vw] md:w-80` wrappers. `GalleryItem` internals are unchanged — cursor-parallax on non-phone cards (mousemove ±12px/10px, scale 1.06×, springs back 0.55s expo-out). Tap triggers `whileTap={{ scale: 1.05, rotateY: 6 }}` spring via a `perspective: 900px` parent div. Clicking any card opens `GalleryLightbox`: a `createPortal` overlay at `z-[500]` with `rotateX: 18→0 + scale: 0.74→1 + blur: 20px→0` spring entrance. ESC in the lightbox uses `stopImmediatePropagation` (capture phase) so it closes only the lightbox, not the underlying modal.
- **Modal prev/next navigation**: `ProjectModal` accepts `prevProject?`, `nextProject?`, `onPrev?`, `onNext?`, `projectIndex?`, `projectCount?` props. When at least one of `onPrev`/`onNext` is provided, a footer bar renders at the bottom of the panel with labeled chevron buttons and a `NN / NN` counter. Buttons disable (opacity 30%, cursor-not-allowed) at boundaries. Project content cross-fades via `AnimatePresence mode="wait"` keyed on `project.slug` — the article re-animates (opacity 0→1 in 0.18s) on each prev/next navigation.
- **Work section no longer uses `Stagger`** for project cards. Instead it uses `AnimatePresence mode="popLayout"` on individual `motion.div` cards, each with `whileInView` for scroll-reveal and `exit` for filter transitions. `Stagger` is still used in FAQ, Capabilities, Packages, and other sections — only Work was migrated.
...
| `Work` section filter tabs | Category filter pills (All / Web / Mobile / Game) above the project grids. Uses `layoutId="filter-pill"` Motion spring to slide an `absolute bg-accent rounded-full` indicator between the active button. State is `FilterCategory = 'all' \| ProjectCategory`, local to `Work.tsx`. Filtering updates `activeCategories` and `visibleProjects` (the last drives prev/next modal navigation order). |
| `Work` category rows | Projects are grouped into category rows (Web → Mobile → Game) via `CATEGORY_ROWS` constant. When filter is `'all'`, all rows render with a hairline label separator (`— Web —`). When filtering to a specific category, only that row renders (label hidden). Each row is a uniform `grid grid-cols-2 gap-5 md:gap-6`. Category rows animate in/out via `AnimatePresence mode="popLayout"` on the row-level `motion.div`. Individual cards animate in via `whileInView` (blur+y resolve). Web/Game covers are `aspect-[16/10]` (landscape); Mobile covers are `aspect-[3/4]` (portrait) so phone screenshots inside `PhoneCoverPreview` display at a natural device size instead of being squished into a pill. Gap filler renders in any category with an odd count > 1. |
| `Work` card cover aspect | Web/Game cards use `aspect-[16/10]` landscape. Mobile cards use `aspect-[3/4]` portrait — the row is internally uniform, and grid alignment is preserved because each category lives in its own row. Ken Burns still applies to featured-flagged web/game covers. The `max-h-[260px]` cap on `PhoneCoverPreview`'s `PhoneFrame` was removed when mobile covers went portrait so the frame fills the available height. Card body text is slightly smaller on mobile (`text-sm / text-xs`) to respect the 2-col mobile layout. |
| `PendulumToggle` visibility | The pendulum control in `Hero.tsx` is `hidden md:flex` — hidden on mobile, visible on desktop. Mobile users don't see the pendulum button or Newton's Cradle strip. |
| `ProjectModal` scroll-to-top | When `project.slug` changes (prev/next navigation), a `useEffect` in `ProjectModal` sets `document.querySelector('[data-lenis-prevent]').scrollTop = 0` instantly so the user sees the hero cover and title of the next project without a scroll position carry-over. |
...
- `--ink-quiet` dark: `#828280` (~5.3:1 on dark bg). `--ink-quiet` light: `#6E6D68` (~4.8:1 on light bg). Both bumped for WCAG AA compliance on mobile where `CursorGlow` doesn't run to lift text. `--ink-soft` stays at `#B5B4AE` (dark) / `#5A5A5A` (light).
...
| `scrollbar-none` | Hides the scrollbar on filmstrip scroll containers (`scrollbar-width: none` + `::-webkit-scrollbar { display: none }`). Used on the Work featured strip and the modal gallery strip. |
...
- ✅ **Phase 2** — cinematic motion
  ...
  - ✅ 2.4: Work section revamp — category filter tabs (layoutId sliding pill), AnimatePresence popLayout for filter transitions, gallery horizontal filmstrip in modal, prev/next project navigation in modal, `Highlight` type migration (metric support), `--ink-quiet` dark + light contrast fix, uniform `aspect-[16/10]` cards, category row grouping (Web/Mobile/Game), pendulum hidden on mobile, modal scroll-to-top on prev/next
