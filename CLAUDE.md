...
| `Work` category rows | Projects are grouped into category rows (Web → Mobile → Game) via `CATEGORY_ROWS` constant. When filter is `'all'`, all rows render with a hairline label separator (`— Web —`). When filtering to a specific category, only that row renders (label hidden). Each row is a uniform `grid grid-cols-2 gap-5 md:gap-6`. Category rows animate in/out via `AnimatePresence mode="popLayout"` on the row-level `motion.div`. Individual cards animate in via `whileInView` (blur+y resolve). All card covers are uniformly `aspect-[16/10]` — phone projects still use `PhoneCoverPreview` (phone frames float in the landscape container), but there is no `featured`-driven size difference. Gap filler renders in any category with an odd count > 1. |
| `Work` card size uniformity | All project cards use `aspect-[16/10]` for the cover area regardless of category. The `featured` prop was removed from `ProjectCard`. Ken Burns still applies to featured-flagged web/game covers. Phone covers still render phone frames via `PhoneCoverPreview` (now takes only `project`, not `featured`). Card body text is slightly smaller on mobile (`text-sm / text-xs`) to respect the 2-col mobile layout. |
