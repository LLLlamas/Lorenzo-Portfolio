...
| `Work` section filter tabs | Category filter pills (All / Web / Mobile / Game) above the project grids. Uses `layoutId="filter-pill"` Motion spring to slide an `absolute bg-accent rounded-full` indicator between the active button. State is `FilterCategory = 'all' \| ProjectCategory`, local to `Work.tsx`. Filtering updates `filteredFeatured`, `filteredRest`, and `visibleProjects` (the last drives prev/next modal navigation order). |
| `Work` featured filmstrip | On mobile (`< md`): featured cards render in a horizontal `flex snap-x snap-mandatory overflow-x-auto scrollbar-none` strip; each card is `w-[85vw] snap-start`. A right-edge `bg-gradient-to-l from-bg` peek-fade hints at scroll. On desktop: normal `grid grid-cols-2`. Non-featured cards are `grid-cols-2` on both mobile and desktop (previously single-column on mobile). |
...
- `--ink-quiet` in dark mode is `#828280` (bumped from original `#6E6E68` for WCAG AA contrast on mobile, where `CursorGlow` doesn't run to lift text. ~5.3:1 on dark bg). `--ink-soft` stays at `#B5B4AE`.
...
- ✅ **Phase 2** — cinematic motion
  ...
  - ✅ 2.4: Work section revamp — category filter tabs (layoutId sliding pill), mobile horizontal scroll-snap filmstrip for featured projects, AnimatePresence popLayout for filter transitions, gallery horizontal filmstrip in modal, prev/next project navigation in modal, `Highlight` type migration (metric support), `--ink-quiet` dark contrast fix
