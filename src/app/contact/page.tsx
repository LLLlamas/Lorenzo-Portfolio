    ...
    <Card as="section" className="p-6 md:p-8">
      ...
      >
        ...
        <div className="mt-2">
          <button
            type="submit"
            className="btn-sweep group inline-flex h-12 items-center justify-center rounded-full border border-accent bg-accent px-6 text-[15px] font-medium tracking-tight text-accent-on transition-colors hover:text-bg hover:border-ink"
            style={{ ['--sweep-bg' as never]: 'var(--ink)' }}
          >
            Send note
          </button>
        </div>
...
function Field({
  ...
  return (
    <label className="block">
      <span className="mb-1.5 block text-xs font-bold uppercase tracking-[0.16em] text-ink-quiet">
        {label}
      </span>
      {as === 'textarea' ? (
        <textarea
